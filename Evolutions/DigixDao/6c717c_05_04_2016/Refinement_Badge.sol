...
contract Refinement {
    ...
    /// @notice precondition forall (address addr) od.users[addr].badges == nw.balances[addr] // Abs func 
    /// @notice precondition od.users[_owner].badges == _balance_
    ...
    /// @notice precondition forall (address addr) od.users[addr].badges == nw.balances[addr] // Abs func 
    /// @notice precondition forall (address addr) od_old.users[addr].badges == nw_old.balances[addr] // Abs func 
    /// @notice precondition (( od.users[msg.sender].badges == od_old.users[msg.sender].badges - value  && msg.sender != to) || (od.users[msg.sender].badges == od_old.users[msg.sender].badges && msg.sender == to ) && _success_ ) || !_success_
    /// @notice precondition (( od.users[to].badges == od_old.users[to].badges + value && msg.sender != to ) || ( od.users[to].badges == od_old.users[to].badges && msg.sender == to ) && _success_ ) || !_success_
