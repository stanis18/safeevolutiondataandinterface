...
contract ERC721A is IERC721A {
     ...
     */
    /// @notice postcondition _packedAddressData[owner]  == balance
    function balanceOf(address owner) public view   returns (uint256 balance) {
        if (owner == address(0)) revert ();
        return _packedAddressData[owner];
    }
    ...
    // / @notice postcondition _packedOwnerships[tokenId] == _owner
    ...
    /// @notice postcondition _tokenApprovals[tokenId].value == to
    ...
    /// @notice postcondition _tokenApprovals[tokenId].value == approved
    ...
    function setApprovalForAll(address operator, bool approved) public   {
        if (operator == _msgSenderERC721A()) revert ();
    ...
    /// @notice  postcondition ( ( _packedAddressData[from] ==  __verifier_old_uint (_packedAddressData[from] ) - 1  &&  from  != to ) || ( from == to )  ) 
    /// @notice  postcondition ( ( _packedAddressData[to] ==  __verifier_old_uint ( _packedAddressData[to] ) + 1  &&  from  != to ) || ( from  == to ) )
    // / @notice  postcondition  _packedOwnerships[tokenId] == to
    ...
    //  */
    // /// @notice emits ConsecutiveTransfer 
    // function _mintERC2309(address to, uint256 quantity) internal  {
    //     uint256 startTokenId = _currentIndex;
    //     if (to == address(0)) revert ();
    //     if (quantity == 0) revert ();
    //     if (quantity > MAX_MINT_ERC2309_QUANTITY_LIMIT) revert ();
